...
export default function Home() {
  const [is_auth, setis_auth] = createSignal(true);
    ...
    <>
      <div class="bg-gray-100 p-4 flex flex-col justify-center items-center w-screen h-screen">
        <div class="w-96 h-80 bg-white rounded-lg shadow p-6 flex flex-col justify-around">
          ...
          <button onClick={handleButtonClick} class="underline text-blue-600">
            {is_auth() ? "Login Via KeyCode" : "Login Via Auth"}
          </button>
    ...
    <>
      <form class="space-y-4 flex flex-col justify-around items-center">
        <input
          type="password"
          placeholder="Key"
          class="block w-[80%] px-4 py-2 text-gray-700 border rounded-lg appearance-none focus:outline-none focus:border-blue-300 bg-gray-100"
        />
        <button
          class="w-48 h-12 text-white transition duration-200 ease-out bg-blue-500 hover:bg-blue-600 rounded-lg self-center"
          type="submit"
        >
          Login
        </button>
